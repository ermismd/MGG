...
public class CreateMGGVisualStyleTask extends AbstractTask {
	...
	public void run(TaskMonitor taskMonitor) throws Exception {
		...
		DiscreteMapping<String, LineType> edgeTypeMapping = (
				DiscreteMapping<String, LineType>) discreteMappingFactory.createVisualMappingFunction(columnEdgeType, String.class, et);
	   ...
	   ContinuousMapping<Double, Paint> edgeColorMapping = (ContinuousMapping<Double, Paint>) 
	    ...
	    BoundaryRangeValues<Paint> neutralRange = new BoundaryRangeValues<>(Color.DARK_GRAY, Color.DARK_GRAY, Color.DARK_GRAY); // for values from 0 to 0.3
	    ...
	    // IMPORTANT NOTE:
	    // In order for the default to work on the edges without a weight, we need to set it AFTER we define the continuous mapping edgeColorMapping 
		style.setDefaultValue(BasicVisualLexicon.EDGE_STROKE_UNSELECTED_PAINT, Color.BLACK);
	...
	private Map<String, LineType> getEdgeTypeMap(){
		...
		edgeTypeMap.put("complementarity", LineTypeVisualProperty.LONG_DASH);
		edgeTypeMap.put("co-occurrence", LineTypeVisualProperty.SOLID);
		edgeTypeMap.put("co-exclusion", LineTypeVisualProperty.SOLID);
		return edgeTypeMap;
	}
